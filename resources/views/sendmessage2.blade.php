...
<form action="{{ route('sendmessagep') }}" method="post">
    @csrf
    <input hidden name="send_to" value="certain">
    <div class="form-group">
        <label>Send to:</label>
        <select multiple="multiple" name="recips[]" class="form-control">
            @foreach($users as $u)
            <option value="{{$u->subscriber_id}}">{{$u->name}}</option>
            @endforeach
            <!--<option value="1">John Thor</option>
            <option value="2">John Wick</option>
    ...
    <div class="row">
        <div class="form-group col">
            <label>Choose a message to send to the chosen subscribers:</label>
